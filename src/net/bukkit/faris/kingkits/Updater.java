...
public class Updater {
	...
	private String[] noUpdateTag = { "-DEV", "-PRE", "-SNAPSHOT", "special" }; // If the version number contains one of these, don't update.
	...
	private boolean versionCheck(String title) {
		if (type != UpdateType.NO_VERSION_CHECK) {
			...
			if (title.split(" v").length == 2) {
				String theTitle = title.split(" v")[0];
				...
				if (hasTag(version) || version.equalsIgnoreCase(remoteVersion) || curVer >= remVer || theTitle.toLowerCase().contains("special")) {
