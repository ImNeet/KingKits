...
import net.bukkit.faris.kingkits.listeners.commands.KingKitsCommand;
import net.bukkit.faris.kingkits.listeners.commands.KitCommand;
import net.bukkit.faris.kingkits.listeners.commands.RefillCommand;
import net.bukkit.faris.kingkits.listeners.commands.RenameKitCommand;
import net.bukkit.faris.kingkits.listeners.event.PlayerListener;
...
public class KingKits extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (this.getCommand("kingkits") != null) {
			...
			this.getCommand("kingkits").setAliases(this.getList("kk"));
		...
		if (this.getCommand("refill") != null) {
			...
			this.getCommand("refill").setAliases(this.getList("soup"));
		...
		if (this.configValues.checkForUpdates) {
			this.updates = new Updates(this, "http://dev.bukkit.org/server-mods/kingkits/files.rss");
			if (!this.updates.isUpdated()) {
				String title = "============================================";
				String titleSpace = "                                            ";
				...
				this.logger.info(title);
				this.logger.info("A new version is available: " + "KingKits v" + this.updates.getVersion());
				this.logger.info("Your current version: KingKits v" + this.getDescription().getVersion());
				if (this.configValues.automaticUpdates) {
					// Automatically update
					this.logger.info("Downloading version " + this.updates.getVersion());
					new Updater(this, "kingkits", this.getFile(), Updater.UpdateType.NO_VERSION_CHECK, false);
				} else {
					this.logger.info("Download it from: " + this.updates.getLink());
				}
			}
		}
	}
	...
	public void onDisable() {
		...
		// Update scores
		this.getScoresConfig().set("Scores", this.playerScores);
	...
	public void loadConfiguration() {
		try {
			...
			this.getConfig().addDefault("PvP Worlds", this.getList("All"));
	...
	private void loadKillstreaks() {
		...
		if (this.getKillstreaksConfig().getBoolean("First run")) {
			this.getKillstreaksConfig().set("Killstreak 9001", this.getList("broadcast &c<player>&a's killstreak is over 9000!"));
